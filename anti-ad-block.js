function showAdBlockOverlay() {
    ...
    adBlockOverlay.className = 'fixed inset-0 flex flex-col items-center justify-center bg-black bg-opacity-100 z-[9999] text-center p-8'; // Dark background overlay
    ...
    adBlockOverlay.innerHTML = `
      <div class="bg-gray-800 dark:bg-gray-900 text-white rounded-2xl p-8 shadow-2xl max-w-sm sm:max-w-md w-full animate-fade-in border-4 border-transparent bg-gradient-to-r from-red-500 via-yellow-500 to-red-500">
        <i data-lucide="alert-triangle" class="w-12 h-12 text-error dark:text-red-300 mx-auto mb-4" aria-hidden="true"></i>
        <h2 id="adblock-title" class="text-2xl sm:text-3xl font-bold text-primary mb-4">Ad Blocker Detected</h2>
        <p id="adblock-description" class="text-sm sm:text-base text-gray-700 dark:text-gray-300 mb-6">Please disable your ad blocker to continue accessing CrazyAlts. Ads help us keep this service free!</p>
        <div class="flex flex-col space-y-4">
          ...
          <button onclick="detectAdBlock()" class="bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 px-6 py-3 rounded-xl hover:bg-gray-300 dark:hover:bg-gray-500 transition-all duration-200 text-lg font-semibold focus:ring-4 focus:ring-gray-400/50 dark:focus:ring-gray-500/50 sm:text-xl">Check Again</button>
        </div>
      </div>
    `;
